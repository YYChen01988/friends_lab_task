...
class TestFriends < MiniTest::Test
  ...
  def test_add_friend()
    result = add_friend(@person5, "Scrappy-Doo")
    assert_equal(1, @person5[:friends].length())
    # .length() can be relace by .size() in this case
  end
  ...
  # .length() can be relace by .count() in this case

  # 6. Find the total of everyone's money
  # (hint: use the @people array, remember how we checked the total number of eggs yesterday?)

  def test_total_of_everybody_money()
    result = total_of_everybody_money()
  ...
  def test_money_loan()
    lender_initial_balance = @person4[:monies]
    lendee_initial_balance = @person5[:monies]
    money_loan(@person4, @person5, 20)
    assert_equal(lender_initial_balance - 20, @person4[:monies])
    assert_equal(lendee_initial_balance +20, @person5[:monies])

  ##Another Answer
    # money_loan(@person5, @person1, 50)
    # assert_equal(50, @person5[:moneies])
    # assert_equal(51, @person1[:moneies])
...
## Another Answer
  ...
  def test_find_friend_with_no_friends()
    result= find_friend_with_no_friends(@people)
